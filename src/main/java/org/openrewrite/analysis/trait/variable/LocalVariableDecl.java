...
class LocalVariableDeclBase extends Top.Base implements LocalVariableDecl {
    ...
    static Validation<TraitErrors, Callable> findNearestParentCallable(Cursor cursor) {
        ...
        while (path.hasNext()) {
            Cursor c = path.next();
            Tree t = c.getValue();
            if (t instanceof J.ClassDeclaration || t instanceof J.NewClass class1 && previous != null && class1.getBody() == previous.getValue()) {
                break;
            }
            if (t instanceof J.Block block && J.Block.isStaticOrInitBlock(c)) {
                if (block.isStatic()) {
